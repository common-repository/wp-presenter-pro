	...
	public function output_options() {
		...
		if ( isset( $_POST['wppp_options_nonce'] ) ) {
			...
			if ( wp_verify_nonce( $post_vars['wppp_options_nonce'], 'save_wppp_options' ) && current_user_can( 'manage_options' ) ) {
				...
				$options['gradients']      = sanitize_text_field( $post_vars['wppp-gradients-select'] );
		...
		<div class="wrap">
			...
			<form method="POST">
				...
				<fieldset>
					<legend><h3><?php esc_html_e( 'Gradient Selection', 'wp-presenter-pro' ); ?></h3></legend>
					<p class="description"><?php esc_html_e( 'Requires the Gutenberg plugin 7.3 or later.', 'wp-presenter-pro' ); ?></p>
					<input type="radio" name="wppp-gradients-select" id="wppp-radio-gradients-on" value="on" <?php checked( 'on', $options['gradients'] ); ?> />&nbsp;<label for="wppp-radio-gradients-on"><?php esc_html_e( 'Enable Selection of Gradients', 'wp-presenter-pro' ); ?></label>
					<br />
					<input type="radio" name="wppp-gradients-select" id="wppp-radio-gradients-off" value="off" <?php checked( 'off', $options['gradients'] ); ?> />&nbsp;<label for="wppp-radio-gradients-off"><?php esc_html_e( 'Disable Selection of Gradients', 'wp-presenter-pro' ); ?></label>
				</fieldset>
				<?php submit_button(); ?>
				</form>
				<?php
